...
public class PartyController {
    ...
    @GetMapping("/paged")
    public ResponseEntity<ApiResponse<List<PartyPostResponse>>> getPartyPostsPaged(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate deadline,
            @RequestParam(required = false) Boolean isClosed) {

        List<PartyPostResponse> posts = partyService.getPartyPostsPaged(page, size, deadline, isClosed);
        return ResponseEntity.ok(ApiResponse.success(posts));
    }

    @Operation(summary = "일행 모집 글 상세 조회", description = "특정 일행 모집 글의 상세 정보를 조회합니다")
    @GetMapping("/{postId}")
    public ResponseEntity<ApiResponse<PartyPostDetailReseponse>> getPartyPostDetail(
            @PathVariable Long postId,
            @AuthenticationPrincipal User user){
        PartyPostDetailReseponse dto = partyService.getPartyPostDetail(postId, user);
    ...
    public ResponseEntity<ApiResponse<List<PartyJoinResponse>>> getJoinRequestsByWriter(
            @AuthenticationPrincipal User user) {
        List<PartyJoinResponse> joins = partyService.getJoinRequestsByWriter(user.getId());
    ...
    public ResponseEntity<ApiResponse<List<PartyJoinResponse>>> getMyJoinStatus(
            @AuthenticationPrincipal User user) {
        List<PartyJoinResponse> joins = partyService.getMyResolvedJoins(user.getId());
        return ResponseEntity.ok(ApiResponse.success(joins));
    }
